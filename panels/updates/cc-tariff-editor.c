...
G_DEFINE_TYPE (CcTariffEditor, cc_tariff_editor, ADW_TYPE_ACTION_ROW)
...
static void on_time_value_changed_cb (GtkAdjustment *adjustment, CcTariffEditor *self);
              ...
              GError         **error)
{
  ...
  g_signal_handlers_block_by_func (self->adjustment_from_hours, on_time_value_changed_cb, self);
  g_signal_handlers_block_by_func (self->adjustment_from_minutes, on_time_value_changed_cb, self);
  g_signal_handlers_block_by_func (self->adjustment_to_hours, on_time_value_changed_cb, self);
  g_signal_handlers_block_by_func (self->adjustment_to_minutes, on_time_value_changed_cb, self);
  ...
  g_signal_handlers_unblock_by_func (self->adjustment_from_hours, on_time_value_changed_cb, self);
  g_signal_handlers_unblock_by_func (self->adjustment_from_minutes, on_time_value_changed_cb, self);
  g_signal_handlers_unblock_by_func (self->adjustment_to_hours, on_time_value_changed_cb, self);
  g_signal_handlers_unblock_by_func (self->adjustment_to_minutes, on_time_value_changed_cb, self);
...
update_adjustments (CcTariffEditor *self)
{
  ...
  if (self->clock_format == G_DESKTOP_CLOCK_FORMAT_24H)
  ...
  else
    {
      ...
      gtk_widget_set_visible (GTK_WIDGET (self->stack_from), true);

      if (self->time_period_from == AM)
        gtk_stack_set_visible_child (GTK_STACK (self->stack_from), self->button_from_am);
      else
        gtk_stack_set_visible_child (GTK_STACK (self->stack_from), self->button_from_pm);
  ...
  if (self->clock_format == G_DESKTOP_CLOCK_FORMAT_24H)
  ...
  else
    {
      ...
      gtk_widget_set_visible (GTK_WIDGET (self->stack_to), true);

      if (self->time_period_to == AM)
        gtk_stack_set_visible_child (GTK_STACK (self->stack_to), self->button_to_am);
      else
        gtk_stack_set_visible_child (GTK_STACK (self->stack_to), self->button_to_pm);
...
static void
on_time_value_changed_cb (GtkAdjustment *adjustment,
                          CcTariffEditor *self)
{
...
cc_tariff_editor_class_init (CcTariffEditorClass *klass)
{
  ...
  gtk_widget_class_bind_template_callback (widget_class, on_time_value_changed_cb);
